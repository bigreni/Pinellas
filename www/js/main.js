    ...
    if (/(android)/i.test(navigator.userAgent)) {
        admobid = { // for Android
            ...
            interstitial:'ca-app-pub-9249695405712287/9656270427'
        };
    } else if(/(ipod|iphone|ipad)/i.test(navigator.userAgent)) { // for ios
    admobid = {
      banner: 'ca-app-pub-1683858134373419/7790106682', 
      interstitial: 'ca-app-pub-9249695405712287/1894056826'
    };
  }
   ...
   function checkFirstUse()
    {
        $(".dropList").select2();
        window.ga.startTrackerWithId('UA-88579601-19', 1, function(msg) {
...
function askRating()
{
  ...
  storeAppURL: {
                ios: '1227172674',
                android: 'market://details?id=com.pinellas.free'
               }
};
...
function getDirections() {
    reset();
    var url = encodeURI("http://ridepsta.net/bustime/map/getDirectionsStopsForRoute.jsp?route=" + $("#MainMobileContent_routeList").val());
...
{
    reset();
    var url = encodeURI("http://ridepsta.net/bustime/map/getStopsForRouteDirection.jsp?route=" + $("#MainMobileContent_routeList").val() + "&direction=" + $("#MainMobileContent_directionList").val());
...
function getArrivalTimes() {
    ...
    var url = encodeURI("http://ridepsta.net/bustime/eta/getStopPredictionsETA.jsp?route=" + $("#MainMobileContent_routeList").val() + "&stop=" + $("#MainMobileContent_stopList").val());
    if (allRoutes != null) {
        if (allRoutes.checked) {
            url = encodeURI("http://ridepsta.net/bustime/eta/getStopPredictionsETA.jsp?route=all&stop=" + $("#MainMobileContent_stopList").val());
        }
    }
